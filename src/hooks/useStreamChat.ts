          ...
          },
          {
            onChunk: ({
              ...
              if (updatedMessages) {
                ...
                // content of messages still in the payload (mid-stream
                // compaction in local_agent_handler does this, as does the
                // non-tail-patch escalation in sendResponseChunk). If the
                // payload identifies the active streaming message, recompute
                // its parser state fresh — keeping parser state DEFINED
                // across the replacement avoids the brief render-path swap
                // in DyadMarkdownParser (closedBlocks subtree →
                // fallbackBlocks fragment list), which manifests as the
                // response visibly vanishing for a frame.
                const streamingMsg =
                ...
                if (streamingMessageId !== undefined && streamingMsg) {
                  ...
                  setStreamingBlocksById((prev) =>
                    prev.size === 0
                      ...
                      : new Map<
                          number,
                          ReturnType<typeof initialParserState>
                        >(),
                  );
                }
              } else if (
