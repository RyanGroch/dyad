...
/**
 * Debounce window for streaming-patch acks. Renderer-side coalescing — we
 * only need the *latest* applied seq per chat to measure IPC backlog, so we
 ...
 * `StreamingPatchSchema` carried `seq`, or callers that splice patches
 * outside the drift-tracker pipeline).
 */
