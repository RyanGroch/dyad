...
function toJsonSchema(input: unknown): Record<string, any> {
  ...
  // Zod schema detection: presence of `_def` and `parse` is a strong signal.
  const maybeZod = input as { _def?: unknown; parse?: unknown };
  if (maybeZod._def && typeof maybeZod.parse === "function") {
    try {
      const { zodToJsonSchema } = require("zod-to-json-schema");
      const converted = zodToJsonSchema(input as z.ZodType<any>, {
        target: "openApi3",
      });
