...
export async function handleLocalAgentStream(
  ...
  // tool XML previews so prefix-shifting buildXml deltas don't flood the
  // renderer.
...
function sendResponseChunk(
  ...
  if (sendFullMessages) {
    ...
    // mid-stream. Escalate to a fullMessages send so the renderer
    // authoritatively replaces content and resets parser state. Patches
    // stay reserved for true append-only tail growth.
